...
NTSTATUS CreateStandardSCAndACL(OUT PSECURITY_DESCRIPTOR* SecurityDescriptor, OUT PACL* Acl)
{
	...
	pAdminSid->IdentifierAuthority = SECURITY_NT_AUTHORITY;
	...
	pSystemSid->IdentifierAuthority = SECURITY_NT_AUTHORITY;
	...
	PACL pACL = (PACL)ExAllocatePoolWithTag(PagedPool, ulACLLength, 'SloV');
	if (!pACL)
	{
		ExFreePool(sd);
		return STATUS_INSUFFICIENT_RESOURCES;
...
NTSTATUS GrantAccess(HANDLE hSection, IN PACL StandardAcl)
{
	...
	{
		DbgMsg("NtQuerySecurityObject fail! Status: %p Need size: %u\n", ntStatus, ulNeedSize);
		return ntStatus;
	}
	...
	if (!sd)
	{
		DbgMsg("ExAllocatePoolWithTag fail! Status: %p\n", ntStatus);
		return STATUS_INSUFFICIENT_RESOURCES;
	}
	...
	if (!NT_SUCCESS(ntStatus))
	{
		DbgMsg("NtQuerySecurityObject fail! Status: %p\n", ntStatus);
		ExFreePool(sd);
		return ntStatus;
	}
	...
	if (!NT_SUCCESS(ntStatus))
	{
		DbgMsg("RtlGetDaclSecurityDescriptor fail! Status: %p\n", ntStatus);
		ExFreePool(sd);
		return ntStatus;
	}
	...
	if (!NT_SUCCESS(ntStatus))
	{
		DbgMsg("RtlGetOwnerSecurityDescriptor fail! Status: %p\n", ntStatus);
		ExFreePool(sd);
		return ntStatus;
	}
	...
	if (ntStatus == STATUS_BUFFER_TOO_SMALL)
	{
		...
		if (!NT_SUCCESS(ntStatus))
		{
			DbgMsg("RtlSelfRelativeToAbsoluteSD2 fail! Status: %p\n", ntStatus);
			ExFreePool(sd);
			return ntStatus;
		}
	}
	...
	if (!NT_SUCCESS(ntStatus))
	{
		DbgMsg("RtlSetOwnerSecurityDescriptor fail! Status: %p\n", ntStatus);
		ExFreePool(sd);
		return ntStatus;
	}
	...
	if (!NT_SUCCESS(ntStatus))
	{
		DbgMsg("RtlSetDaclSecurityDescriptor fail! Status: %p\n", ntStatus);
		ExFreePool(sd);
		return ntStatus;
	}
	...
	if (!NT_SUCCESS(ntStatus))
	{
		DbgMsg("NtSetSecurityObject fail! Status: %p\n", ntStatus);
		ExFreePool(sd);
		return ntStatus;
	}
